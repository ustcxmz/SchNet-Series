...
class UpdateV(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, num_filters):
        ...
        self.lin = Linear(hidden_channels, num_filters)
        self.conv = torch.nn.Conv1d(
            num_filters, hidden_channels, kernel_size=3, padding=1
        )
        self.pool = torch.nn.MaxPool1d(kernel_size=2, padding=1)
...
class UpdateV(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, num_filters):
        ...
        self.pool = torch.nn.MaxPool1d(kernel_size=2, stride=1, padding=1)  # Adjust stride and padding
        self.norm = torch.nn.BatchNorm1d(hidden_channels)
        self.reset_parameters()
    ...
    def forward(self, v, e, edge_index):
        ...
        out = torch.zeros_like(v).scatter_add_(
            dim=0, index=i.unsqueeze(-1).expand(-1, e.size(-1)), src=e
        )
        out = self.lin(out)  # 输出形状为 [N, num_filters]

        # 调整形状以适配 Conv1d
        out = out.transpose(0, 1).unsqueeze(0)  # [1, num_filters, N]

        for _ in range(3):
            out = self.conv(out)  # [1, hidden_channels, N]
            out = self.pool(out)  # [1, hidden_channels, N] (stride=1, padding=1)
            out = self.norm(out)  # [1, hidden_channels, N]
            out = out.squeeze(0).transpose(1, 0)  # [N, hidden_channels]
            out = v + out  # 外部残差连接
...
class SingleSchNetPlusPlus(torch.nn.Module):
    """Complete SchNetPlusPlus model implementation with deep MLP and ResNet optimization"""
    ...
    def __init__(
        ...
        super(SingleSchNetPlusPlus, self).__init__()
        self.cutoff = cutoff
...
class SchNetPlusPlus(torch.nn.Module):
    def __init__(
        ...
        self.SingleSchNetPlusPlus = SingleSchNetPlusPlus(
            cutoff,
            num_layers,
            hidden_channels,
            out_channels,
    ...
    def forward(self, batch_data):
        energy = self.SingleSchNetPlusPlus(batch_data)
        for i in range(self.num_head - 1):
            energy += self.SingleSchNetPlusPlus(batch_data)
        return energy / self.num_head
